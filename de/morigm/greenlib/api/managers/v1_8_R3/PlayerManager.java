package de.morigm.greenlib.api.managers.v1_8_R3;
...
import de.morigm.greenlib.api.packet.playout.PacketPlayOutAttachEntity;
...
import net.minecraft.server.v1_8_R3.PacketPlayOutCombatEvent.EnumCombatEventType;
...
public class PlayerManager extends de.morigm.greenlib.api.managers.PlayerManager {
		...
		ch.pipeline().addAfter("decoder", "SignListener", new MessageToMessageDecoder<Packet<?>>()
		{
			...
			protected void decode(ChannelHandlerContext arg0, Packet<?> packet, List<Object> arg2) throws Exception 
			{
				...
				if(packet instanceof PacketPlayInUpdateSign)
				{
					...
					if(ppius.b()[0] != null)
					{
						st[0] = ppius.b()[0].getText();
					}
					if(ppius.b()[1] != null)
					{
						st[1] = ppius.b()[1].getText();
					}
					if(ppius.b()[2] != null)
					{
						st[2] = ppius.b()[2].getText();
					}
					if(ppius.b()[3] != null)
					{
						st[3] = ppius.b()[3].getText();
					}
					new BukkitRunnable() 
					{
	...
	public void sendEquipment(Entity e, EquipmentType type, ItemStack item) 
	{
		...
		{
			PacketPlayOutEntityEquipment packet = new PacketPlayOutEntityEquipment(e.getEntityId(), 0, CraftItemStack.asNMSCopy(item));
			CraftPlayer cp = (CraftPlayer) player;
			cp.getHandle().playerConnection.sendPacket(packet);
		}
		if(type.equals(EquipmentType.FEET))
		{
			PacketPlayOutEntityEquipment packet = new PacketPlayOutEntityEquipment(e.getEntityId(), 1, CraftItemStack.asNMSCopy(item));
			CraftPlayer cp = (CraftPlayer) player;
			cp.getHandle().playerConnection.sendPacket(packet);
		}
		if(type.equals(EquipmentType.LEGS))
		{
			PacketPlayOutEntityEquipment packet = new PacketPlayOutEntityEquipment(e.getEntityId(), 2, CraftItemStack.asNMSCopy(item));
			CraftPlayer cp = (CraftPlayer) player;
			cp.getHandle().playerConnection.sendPacket(packet);
		}
		if(type.equals(EquipmentType.CHEST))
		{
			PacketPlayOutEntityEquipment packet = new PacketPlayOutEntityEquipment(e.getEntityId(), 3, CraftItemStack.asNMSCopy(item));
			CraftPlayer cp = (CraftPlayer) player;
			cp.getHandle().playerConnection.sendPacket(packet);
		}
		if(type.equals(EquipmentType.HEAD))
		{
			PacketPlayOutEntityEquipment packet = new PacketPlayOutEntityEquipment(e.getEntityId(), 4, CraftItemStack.asNMSCopy(item));
			CraftPlayer cp = (CraftPlayer) player;
			cp.getHandle().playerConnection.sendPacket(packet);
		}
	}
	...
	public void sendPacket(PacketPlayOut packet) 
	{
		...
		if(packet instanceof PacketPlayOutAbilities)
		{
			...
			pack = new net.minecraft.server.v1_8_R3.PacketPlayOutAbilities(pa);
		}
		if(packet instanceof PacketPlayOutAnimation)
		{
			PacketPlayOutAnimation pa = (PacketPlayOutAnimation) packet;
			pack = new net.minecraft.server.v1_8_R3.PacketPlayOutAnimation(((CraftEntity)pa.a()).getHandle(), pa.b());
		}
		if(packet instanceof PacketPlayOutAttachEntity)
		{
			PacketPlayOutAttachEntity pa = (PacketPlayOutAttachEntity) packet;
			pack = new net.minecraft.server.v1_8_R3.PacketPlayOutAttachEntity(pa.c(), ((CraftEntity)pa.a()).getHandle(), ((CraftEntity)pa.b()).getHandle());
		}
		if(packet instanceof PacketPlayOutBed)
		{
			PacketPlayOutBed pa = (PacketPlayOutBed) packet;
			pack = new net.minecraft.server.v1_8_R3.PacketPlayOutBed(((CraftPlayer)pa.a()).getHandle(),new BlockPosition(pa.b().getX(), pa.b().getY(), pa.b().getZ()));
		}
		if(packet instanceof PacketPlayOutBlockAction)
		{
			PacketPlayOutBlockAction pa = (PacketPlayOutBlockAction) packet;
			pack = new net.minecraft.server.v1_8_R3.PacketPlayOutBlockAction(new BlockPosition(pa.a().getX(), pa.a().getY(), pa.a().getZ()),(net.minecraft.server.v1_8_R3.Block) pa.d().getState(),pa.b(),pa.c());
		}
		if(packet instanceof PacketPlayOutBlockBreakAnimation)
		{
			...
			pack = new net.minecraft.server.v1_8_R3.PacketPlayOutBlockBreakAnimation(pa.getEntityID(), new BlockPosition(pa.getLocation().getX(), pa.getLocation().getY(), pa.getLocation().getZ()), pa.getCount());
		}
		if(packet instanceof PacketPlayOutBlockChange)
		{
			PacketPlayOutBlockChange pa = (PacketPlayOutBlockChange) packet;
			pack = new net.minecraft.server.v1_8_R3.PacketPlayOutBlockChange(((CraftWorld)pa.getWorld()).getHandle(),new BlockPosition(pa.getLocation().getX(), pa.getLocation().getY(), pa.getLocation().getZ()));
			((net.minecraft.server.v1_8_R3.PacketPlayOutBlockChange)pack).block = CraftMagicNumbers.getBlock(pa.getMaterial().getId()).fromLegacyData(pa.getData());
		}
		if(packet instanceof PacketPlayOutCamera)
		{
			...
			pack = new net.minecraft.server.v1_8_R3.PacketPlayOutCamera(ce.getHandle());
		}
		if(packet instanceof PacketPlayOutChat)
		{
		    ...
		    for (int j = 0; j < i - 1 ; j++)
		    {
		      ...
		      cp.getHandle().playerConnection.sendPacket(new net.minecraft.server.v1_8_R3.PacketPlayOutChat(component));
		    }
		    IChatBaseComponent component = arrayOfIChatBaseComponent[i - 1];
		    pack = new net.minecraft.server.v1_8_R3.PacketPlayOutChat(component);
		}
		if(packet instanceof PacketPlayOutCloseWindow)
		{
			PacketPlayOutCloseWindow pa = (PacketPlayOutCloseWindow) packet;
			pack = new net.minecraft.server.v1_8_R3.PacketPlayInCloseWindow(pa.getWindow());
		}
		if(packet instanceof PacketPlayOutCollect)
		{
			PacketPlayOutCollect pa = (PacketPlayOutCollect) packet;
			pack = new net.minecraft.server.v1_8_R3.PacketPlayOutCollect(pa.getCollectorEntityID(), pa.getPickupItemCount());
		}
		if(packet instanceof PacketPlayOutCombatEvent)
		{
			...
			pack = new net.minecraft.server.v1_8_R3.PacketPlayOutCombatEvent(new CombatTracker(entityLiving),EnumCombatEventType.valueOf(pa.getType().name().toUpperCase()));
		}
		if(packet instanceof PacketPlayOutCustomPayload)
		{
			PacketPlayOutCustomPayload pa = (PacketPlayOutCustomPayload) packet;
			PacketDataSerializer packetDataSerializer = ((de.morigm.greenlib.api.buffer.v1_8_R3.PacketBuffer)pa.getPacketBuffer()).getPacketDataSerializer();
			pack = new net.minecraft.server.v1_8_R3.PacketPlayOutCustomPayload(pa.getChannel(), packetDataSerializer);
		}
		if(packet instanceof PacketPlayOutEntity)
		{
			PacketPlayOutEntity pa = (PacketPlayOutEntity) packet;
			pack = new net.minecraft.server.v1_8_R3.PacketPlayOutEntity(pa.getEntityId());
		}
		if(packet instanceof PacketPlayOutRelEntityMoveLook)
		{
			...
			byte x =  pa.getDeltaX() > Byte.MAX_VALUE ? Byte.MAX_VALUE : (byte) pa.getDeltaX();
			byte y =  pa.getDeltaY() > Byte.MAX_VALUE ? Byte.MAX_VALUE : (byte) pa.getDeltaY();
			byte z =  pa.getDeltaZ() > Byte.MAX_VALUE ? Byte.MAX_VALUE : (byte) pa.getDeltaZ();
			pack = new net.minecraft.server.v1_8_R3.PacketPlayOutEntity.PacketPlayOutRelEntityMoveLook(pa.getEntityId(), x, y, z, pa.getPitch(), pa.getYaw(), pa.isOnGround());
		}
		if(packet instanceof PacketPlayOutRelEntityMoveLook)
		{
			PacketPlayOutRelEntityMoveLook  pa = (PacketPlayOutRelEntityMoveLook) packet;
			...
			byte y =  pa.getDeltaY() > Byte.MAX_VALUE ? Byte.MAX_VALUE : (byte) pa.getDeltaY();
			byte z =  pa.getDeltaZ() > Byte.MAX_VALUE ? Byte.MAX_VALUE : (byte) pa.getDeltaZ();
			pack = new net.minecraft.server.v1_8_R3.PacketPlayOutEntity.PacketPlayOutRelEntityMove(pa.getEntityId(), x, y, z, pa.isOnGround());
		}
		if(packet instanceof PacketPlayOutEntityLook)
		{
			PacketPlayOutEntityLook pa = (PacketPlayOutEntityLook) packet;
			pack = new net.minecraft.server.v1_8_R3.PacketPlayOutEntity.PacketPlayOutEntityLook(pa.getEntityId(), pa.getPitch(), pa.getYaw(), pa.isOnGround());
		}
		...
		if(packet instanceof PacketPlayOutEntityEffect)
		{
			...
			pack = new net.minecraft.server.v1_8_R3.PacketPlayOutEntityEffect(pa.getParamint(), new MobEffect(pa.getMobEffect().getMobEffectList().getEffectID(), pa.getMobEffect().getI(), pa.getMobEffect().getX(), pa.getMobEffect().isParm1(), pa.getMobEffect().isParm2()));
		}
		if(packet instanceof de.morigm.greenlib.api.packet.playout.PacketPlayOutEntityEquipment)
		{
			...
			pack = new PacketPlayOutEntityEquipment(pa.getEntityID(), pa.getEquipmentType().getID(), CraftItemStack.asNMSCopy(pa.getItemStack()));
		}
